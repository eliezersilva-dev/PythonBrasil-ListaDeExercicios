...
class Bichinho:
    ...
    # método para mostrar valores dos atributos
    def valores_atributos(self):
        valores = str(f'Data Nascimento: {self.nascimento}\n'
                      f'Idade: {self.valor_idade}\n'
                      f'Saúde: {self.valor_saude}\n'
                      f'Fome: {self.valor_fome}\n'
                      f'Humor: {self.valor_humor}')
...
def menu(bichinho):
    ...
    if opcao == '1':
        ...
        print('Obrigado por cuidar de mim!!\n'
              'Adeus :)')
        quit()
    # opção secreta no menu
    elif opcao == '4':
        print('\nOpção secreta - exibir valores exatos dos atributos:')
        bichinho.valores_atributos()
    else:
        menu(bichinho)
